...
def save_data():
    ...
    outdir = f"/root/nav_project/dqn_ros/training_results/results-{timestamp}"
    ...
    # 保存模型权重
    torch.save(policy_net.state_dict(), f"{outdir}/policy_net.pth")
    torch.save(target_net.state_dict(), f"{outdir}/target_net.pth")
    print(f"Model weights saved to {outdir}/")
...
def optimize_model(batch_size, gamma):
    ...
    # Double DQN: policy_net 选动作, target_net 评估
...
if __name__ == '__main__':
    ...
    n_observations = env.observation_space.shape[0]  # 从环境自动获取观测维度
